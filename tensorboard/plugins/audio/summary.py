...
def op(name,
  ...
  Arguments:
    ...
    file_format: A constant `str` (not string tensor) indicating the
  ...
  API compatibility note: The default value of the `file_format`
  argument is _not_ guaranteed to remain unchanged across TensorBoard
  versions. If the specific format is important to you, please provide a
  ...
  if file_format == 'wav':
    ...
    raise ValueError('Unknown file_format: %r' % file_format)
...
def pb(name,
  ...
  Arguments:
    ...
    max_outputs: Optional `int`. At most this many audio clips will be
      ...
      discarded.
    file_format: A constant `str` indicating the desired encoding. You
