...
class SummaryTest(tf.test.TestCase):
  ...
  def test_requires_wav_in_op(self):
    with six.assertRaisesRegex(self, ValueError, 'Unknown file_format'):
      summary.op('k488', self.stereo, 44100, file_format='pptx')

  def test_requires_wav_in_pb(self):
    with six.assertRaisesRegex(self, ValueError, 'Unknown file_format'):
      summary.pb('k488', self.stereo, 44100, file_format='pptx')
